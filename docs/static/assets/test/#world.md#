...
$l_i=\log( m_i(0)/m_i(1))$, $M_i=\log( M_i(0)/M_i(1))$, one can get the
simplified message:
...
In GF(4) code, the generator matrix $G=(A|B)$ satisfies
$G \tilde G^T=AB^T+BA^T=0$, where $\tilde G=(B|A)$. Here, one can just
decode a classical code with parity check matrix $\tilde G$, then check
if it is a combination of rows of $G$.
